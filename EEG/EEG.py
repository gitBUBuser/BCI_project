...
"""
    ...
    This file is responsible for handling EEG input from the port, decoding it and saving it as EDF files.
    Preprocessing methods are also included here.
"""

# EEG_Reader - this class reads EEG signals from the port and puts them into its "sample_buffer"
class EEG_Reader:
...
# LiveEEGRecorder - This lighweight class is used to get live EEG data while running the EEG_reader on a seperate thread to minimize interferance
...
# EEGRecorder, a heavyweight alternative to LiveEEGRecorder, this class includes a recording queue aswell as a buffer that gets updated all the time.
# the main functionality of this class is to allow users to annotate live EEG_data, and save recordings as EDF files.
# the class is expected to run on a seperate thread.
class EEG_recorder:
